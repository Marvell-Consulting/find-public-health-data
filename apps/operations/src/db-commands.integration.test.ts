...
describe('db reset (integration)', () => {
  ...
  it('reset → migrate → import-core-data → seed-dummy-data rebuilds a working database', async () => {
    ...
    try {
      ...
      // Stands in for anything a future migration creates outside the table sweeps; the
      // extensions' functions must survive it or the migrate below would break.
      ...
      expect(await sql`SELECT 1 FROM pg_proc WHERE proname = 'reset_probe'`).toHaveLength(0);
