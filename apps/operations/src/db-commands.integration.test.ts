...
describe('db reset (integration)', () => {
  ...
  it('reset → migrate → import-core-data → seed-dummy-data rebuilds a working database', async () => {
    ...
    try {
      ...
      // Stand-ins for anything a future migration creates outside the table sweeps; the
      // extensions' objects must survive or the migrate below would break. The range also
      // plants constructor functions only its type drop can remove.
