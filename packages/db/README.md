...
```
data/                 Core content (topics.json, ci-methods.json, classifications.json), the
...
```sh
pnpm db:import-core-data         # imports topics, CI methods and classifications from data/
```

Upserts matched on `id`: a rename — even one that changes the slug — updates the row in
place without changing the primary key. Classifications are the exception: seed and import
files name them by slug, and they keep the ids the database gave them, so they upsert on
`slug`. A classification whose slug changes is a new row, and the old one is reported as
orphaned. Rows in the database but absent from the file are reported and left alone, never
deleted. Re-runs are true no-ops (`updated_at` untouched), so the import is safe to run
