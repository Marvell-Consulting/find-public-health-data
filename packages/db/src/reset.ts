...
export async function resetDatabase(sql: postgres.Sql): Promise<void> {
  await sql.begin(async (tx) => {
    ...
    // Functions, procedures and aggregates. Extension members ('e') and internal members
    // ('i', a range type's constructors) are excluded: dropping the former fails, and only
    // its owning object can drop the latter. regprocedure renders the schema-qualified,
    // quoted signature the drop needs — `proname` alone cannot name an overload — so this
    // one goes through `unsafe`.
    ...
    // went with its table above, and a range's multirange goes with its range.
