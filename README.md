...
keys are UUIDv7 (native `uuidv7()` default in PostgreSQL 18) and never appear in a public
URL; the addresses an indicator answers to — its number and the slugs it has been
published under — live in `indicator_alias`. Grants are explicit and read-only:
